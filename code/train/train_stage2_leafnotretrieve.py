# -*- coding: utf-8 -*-
# Stage-2 (Leaf-aware) training:
# 叶子节点 → 只编码 query（不做证据检索）
# 非叶节点 → 编码 (query, Top-K 行)
# 冻结 DeBERTa，仅训练 proj_self/proj_child/gate/norm + 线性头
    ...
    def __init__(self, path: str, max_samples: int=None):
        ...
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                table = r.get("table")
                if table is None:
                    header  = r.get("header"); rows = r.get("rows"); caption = r.get("caption")
...
def rerank(query: str, table: Dict[str,Any], k: int = 10) -> List[str]:
    ...
    scored = []
    for row in linearize_row(table):
        s = 0.0
        for w in toks:
...
# ---------- Leaf-aware Stage-2 Model ----------
class Stage2TreeLeaf(nn.Module):
    """
    叶子：只编码 query
    非叶：编码 (query, Top-K 行)
    冻结 enc；训练 proj_self/proj_child/gate/norm + 线性头
    """
    def __init__(self, node: NodeEncoder, num_labels=2, max_len=256,
        ...
        self.node_mb = node_mb  # 节点编码小批处理，防止 OOM
    ...
    # 1) 收集 (node_id, text, ctx, is_leaf) 与 edges
    def _collect_pairs(self, root: Dict[str,Any], table: Dict[str,Any]) -> Tuple[List[Tuple[int,str,str,bool]], Dict[int,List[int]]]:
        pairs, edges = [], {}
        stack = [(0, root)]
        id_map = {id(root): 0}
        next_id = 1
        while stack:
            ...
            if is_leaf:
                ...
                rows = self.rerank_fn(nd["text"], table, k=self.K_self)
                ctx = self.joiner.join(rows)
            pairs.append((nid, nd["text"], ctx, is_leaf))

            ch_ids = []
            for ch in children:
                cid = id_map.get(id(ch))
                if cid is None:
                    cid = next_id; next_id += 1; id_map[id(ch)] = cid
                ch_ids.append(cid)
                stack.append((cid, ch))
            edges[nid] = ch_ids
        return pairs, edges

    # 2) 一次性（分块）编码所有节点：叶子走单文本，非叶走 pair
    @torch.no_grad()
    def _encode_pairs_once(self, pairs: List[Tuple[int,str,str,bool]]) -> Dict[int, torch.Tensor]:
        device = next(self.parameters()).device
        tok = self.node.tok; enc = self.node.enc
        out: Dict[int, torch.Tensor] = {}

        # 分开两组
        leaves  = [(nid, text)         for (nid, text, ctx, is_leaf) in pairs if is_leaf]
        nonleaf = [(nid, text, ctx)    for (nid, text, ctx, is_leaf) in pairs if not is_leaf]

        # helper: pool
        def masked_mean(hs, mask):
            mask = mask.unsqueeze(-1).float()
            return (hs * mask).sum(1) / mask.sum(1).clamp_min(1e-6)

        # 叶子：只编码 query
        for i in range(0, len(leaves), self.node_mb):
            chunk = leaves[i:i+self.node_mb]
            ids, qa = zip(*chunk) if chunk else ([], [])
            if not qa: break
            batch = tok(list(qa), truncation=True, max_length=self.max_len, padding=True, return_tensors="pt").to(device)
            hs = enc(**batch).last_hidden_state
            vec = masked_mean(hs, batch["attention_mask"])
            vec = F.normalize(vec, p=2, dim=-1)
            for nid, v in zip(ids, vec): out[nid] = v

        # 非叶：编码 (query, ctx)
        for i in range(0, len(nonleaf), self.node_mb):
            chunk = nonleaf[i:i+self.node_mb]
            ids, qa, qb = zip(*chunk) if chunk else ([], [], [])
            if not qa: break
            batch = tok(list(qa), list(qb), truncation=True, max_length=self.max_len, padding=True, return_tensors="pt").to(device)
            hs = enc(**batch).last_hidden_state
            vec = masked_mean(hs, batch["attention_mask"])
            vec = F.normalize(vec, p=2, dim=-1)
            for nid, v in zip(ids, vec): out[nid] = v
    ...
    # 4) forward：每样本只跑一次 encoder（分块）
    def forward(self, claims: List[str], tables: List[Dict[str,Any]]) -> torch.Tensor:
        device = next(self.parameters()).device
        Hs = []
        for claim, table in zip(claims, tables):
            root = claim_to_span_tree(claim)  # 你也可以换成缓存版本
            pairs, edges = self._collect_pairs(root, table)
...
# ---------- Train / Eval ----------
@torch.no_grad()
def evaluate(model: Stage2TreeLeaf, dl, device):
    model.eval()
    total = correct = 0
...
def main():
    ...
    ap.add_argument("--ckpt", default="stage2_tree_leaf.pt")
    ap.add_argument("--val_ratio", type=float, default=0.05)
    ap.add_argument("--max_train_samples", type=int, default=None)
    ap.add_argument("--node_mb", type=int, default=128, help="节点编码的子批大小，防OOM")
    args = ap.parse_args()

    seed_all(args.seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # NodeEncoder + 加载 stage-1 编码器权重（只加载 enc.*）
    ...
    model = Stage2TreeLeaf(node, num_labels=2, max_len=args.max_len,
                           child_method=args.child_method, K_self=args.K_self,
                           node_mb=args.node_mb).to(device)
    ...
    # Train
    print(f"[info] train={len(train_set)}, dev={len(dev_set)}, batch={args.batch_size}, "
          f"accum={args.grad_accum}, K_self={args.K_self}, max_len={args.max_len}, node_mb={args.node_mb}")
